...
namespace wp_control
{
  ...
  void GpsWaypoint::orientcb(custom_msgs::orientationEstimate orient)
  {
    ...
    double k1 = 2*m_distanceP*sqrt( pow(targetYawrate,2) + m_headingP*pow(targetVel,2));
    double k2 = m_headingP*targetVel;
    ...
    command.linear.x = Clamp(targetVel + k1 * longError, 0 , 10.0);
    ...
    if (time > (m_prevTime + 1))
    {
      m_prevTime = time;
       ...
       std::cout << " target yaw rate " << targetYawrate << " cmd " << k2*lateralError << std::endl;
       std::cout << " time lapsed " << (time-m_initTime) << " Time check " << timeint << std::endl;
       //std::cout << " position error " << positionError << std::endl;
       std::cout << " Target velocity " << targetVel << " additional velocity " << k1*longError << std::endl;
