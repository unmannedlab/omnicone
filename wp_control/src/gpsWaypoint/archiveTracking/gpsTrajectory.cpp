...
namespace wp_control
{
    ...
    m_nh("~"), m_speed(0.0), m_useThetaGPS(true), m_prevTime(0.0)
  {
    ...
    m_nh.param("LongDistanceP", m_longDistP, 0.01);
    m_nh.param("LateralDistanceP", m_latDistP, 0.05); 
    
    inputFile.open("/home/dev/launch_scripts/inputTrack.txt");
    ...
    outputFile.open("/home/dev/launch_scripts/outputTrack.txt");
    ...
    while(getline(wptFile, curLine))
    {
      ...
      inputFile << pt.x << "," << pt.y << "," << m_timeStamp << "," << targetYaw <<  std::endl;
      //ROS_INFO("Loaded waypoint %f %f %f", pt.x, pt.y, pt.z);
      m_wpts.push_back(pt);
  ...
  void GpsWaypoint::orientcb(custom_msgs::orientationEstimate orient)
  {
    ...
    double k1 = m_longDistP; //2*m_distanceP*sqrt( pow(targetYawrate,2) + m_headingP*pow(targetVel,2));
    double k3 = m_headingP; //m_headingP*targetVel;
    double k2 = m_latDistP;
    ...
    double headingError = AngleDiff(targetYaw, theta);
    double cmdVel = targetVelY + k1 * longError;
    //if (targetVelY < -0.1){
    //  cmdVel = 2;
    //}

    geometry_msgs::Twist command;
    command.linear.x = Clamp(cmdVel, 0 , 25.0);
    command.angular.z = Clamp(targetYawrate - k2 * lateralError + k3 * headingError,-1.0,1.0);

    m_twist_pub.publish(command);
    
    //outputFile.open("/home/dev/launch_scripts/outputTrack.txt");    
    outputFile << x << "," << y << "," << (time-m_initTime) << "," << theta <<  std::endl;
    //outputFile.close();
