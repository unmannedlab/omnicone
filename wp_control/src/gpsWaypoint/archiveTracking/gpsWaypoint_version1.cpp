...
//Input data is (X,Y,t)
//This code uses proportional control on the difference in the desired yaw and current yaw angles
//No change of frame was used here. 
//The problem with this controller was that if you get close to the target point, the motion is not smooth
//Also, there was no distinction between lateral error and longitudinal error
...
namespace wp_control
{
    ...
    m_nh("~"), m_speed(0.0), m_useThetaGPS(true), m_prevTime(0.0)
  {
    ...
    m_nh.param("VelocityP", m_distanceP, 0.03);
...
//                   &GpsWaypoint::paramCallback, this);
    while(getline(wptFile, curLine))
    {
      ...
      ROS_INFO("Loaded latlon waypoint %f %f %f", latlon_point.latitude, latlon_point.longitude, m_timeStamp);
  ...
  void GpsWaypoint::orientcb(custom_msgs::orientationEstimate orient)
  {
    ...
    double xn = temp_wpts.front().x;
    double yn = temp_wpts.front().y;
    double tn = temp_wpts.front().z;
    bool noTarget = true;
    ...
    while (noTarget)
    {
      temp_wpts.push_back(temp_wpts.front());
      temp_wpts.pop_front();

      if (timeint-tn<0.5) { 
	 noTarget = false;
         targetYaw = atan2(temp_wpts.front().y-yn,temp_wpts.front().x-xn);

         if (temp_wpts.front().z > 0.0){
           targetVel = sqrt( pow(temp_wpts.front().x-xn,2) + pow(temp_wpts.front().y-yn,2) )/abs(temp_wpts.front().z-tn);
         }else{
           targetVel = 0;
         }
         //ROS_INFO("Hit a waypoint");
      }else {
         xn = temp_wpts.front().x;
         yn = temp_wpts.front().y;
         tn = temp_wpts.front().z;
      } 	
    }		
    ...
    if (time > (m_prevTime + 1))
    {
      m_prevTime = time;
       ...
       std::cout << " Target velocity " << targetVel << " additional velocity " << m_distanceP * positionError << std::endl;
